<template>
  <div>
    ...
    <template v-slot:projectName="{data}">
      <el-link type="success" @click.prevent="handleOperation(2, data)">{{ data.projectName }}</el-link>
    </template>
    <template v-slot:type1="{data}">
      <span >{{revenueTypeOptions.filter(v=>v.value == data.type1)[0]?.label}}</span>
    </template>
    <template v-slot:type2="{data}">
      <span >{{revenueTypeOptions.filter(v=>v.value == data.type2)[0]?.label}}</span>
    </template>
...
export default {
  ...
  setup() {
    ...
    const handleOperation = (type, rowData) => {
      ...
      router.push({
        name: 'editEconomics',
        params: { data : encodeURIComponent(data), operation: type, type:'item' },
      })
    }
    //
