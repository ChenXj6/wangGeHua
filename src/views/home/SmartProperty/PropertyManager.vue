<template>
  <div>
    ...
    >
      ...
      <template v-slot:cleanerType="{data}">
        <span>{{ data.cleanerType == 1 ? '保洁' : data.cleanerType == 2 ? '公共秩序员' : data.cleanerType == 3 ? '管家' : data.cleanerType == 4 ? '绿化工' : data.cleanerType == 5 ? '维修工' : data.cleanerType == 6 ? '项目经理' : (data.cleanerType == 7 ? '行政人事' : '女')}}</span>
      </template>
      <template v-slot:sex="{data}">
        <span>{{ data.sex == 1 ? '女' : '男' }}</span>
      </template>
...
import { renderTable } from './common/PropertyManager'
import { deepClone,formatterDate, defaultObject } from '@/utils/util'
import { deleteCleaner } from '@/api/SmartProperty/propertyManager'
export default defineComponent({
  ...
  setup() {
    ...
    const handleOperation = (type, rowData) => {
      ...
      router.push({
        name: 'editsmart',
        params: { data : encodeURIComponent(data), operation: type, type:'Cleaner' },
      })
    }
    const getOList = () => {
