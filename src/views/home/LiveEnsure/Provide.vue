<template>
  <div>
    ...
    >
    <template v-slot:orgName="{data}">
      <el-link type="success" @click.prevent="handleOperation(2, data)">{{ data.orgName }}</el-link>
    </template>
...
import { renderTable } from './common/Provide'
...
export default {
  ...
  setup() {
    ...
    getOptionsByCode(1067,orgTypeOptions)
    onMounted(()=>{
      handleQuery()
    })
    return{
