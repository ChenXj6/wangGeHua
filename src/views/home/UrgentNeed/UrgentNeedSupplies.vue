<template>
  <div>
    ...
    >
    <template v-slot:suppliesName="{data}">
      <el-link type="success" @click.prevent="handleOperation(2, data)">{{ data.suppliesName }}</el-link>
    </template>
...
export default {
  ...
  setup() {
    ...
    const handleOperation = (type, rowData) => {
      ...
      router.push({
        name: 'editUrgentNeed',
        params: { data : encodeURIComponent(data), operation: type, type:'supp' },
      })
    }
    //
